...
public:
	...
	int begin(Device device, esphome::i2c::I2CDevice* i2c_device);
	/* Initialise the hardware, it's the same as begin() */
	int setup(Device device, esphome::i2c::I2CDevice* i2c_device) { return begin(device, i2c_device); }
	...
	static Device probe(esphome::i2c::I2CDevice* i2c_device) {
		Trill t;

		/* Check the type of device attached */
		t.i2c_device_ = i2c_device;
		if (t.identify() != 0) {
...
private:
	...
	uint8_t rx_buffer_[128]; /* Buffer for raw I2C reads */
